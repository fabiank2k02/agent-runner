...
export function parseCodexStatusOutput(output: string, collectedAt = new Date().toISOString()): CodexAccountUsageSnapshot | null {
  ...
  if (parsed && typeof parsed === "object") {
    return normalizeJsonStatus(parsed as Record<string, unknown>, collectedAt);
  }
  return normalizeTextStatus(text, collectedAt);
}

export async function collectCodexAccountUsageSnapshot(options: { timeoutMs?: number } = {}): Promise<CodexAccountUsageSnapshot | null> {
...
function normalizeJsonStatus(value: Record<string, unknown>, collectedAt: string): CodexAccountUsageSnapshot {
...
function normalizeTextStatus(text: string, collectedAt: string): CodexAccountUsageSnapshot | null {
...
function normalizeRemaining(value: Record<string, unknown> | null): Record<string, unknown> | null {
  if (!value) {
    return null;
  }
  const remaining =
    numberAt(value, "remainingTokens") ??
  ...
  const percentRemaining =
    numberAt(value, "percentRemaining") ??
...
function normalizeTokenUsage(value: Record<string, unknown>) {
  ...
  const totalTokens =
    numberAt(value, "totalTokens") ?? numberAt(value, "total_tokens") ?? numberAt(value, "tokens") ?? inputTokens + outputTokens;
  return {
    inputTokens,
    cachedInputTokens,
    outputTokens,
...
function normalizeReset(value: Record<string, unknown>, limits: Record<string, unknown>) {
  return {
    weeklyResetAt:
      stringAt(value, "weeklyResetAt") ||
      stringAt(value, "weekly_reset_at") ||
      stringAt(limits, "weeklyResetAt") ||
      stringAt(limits, "weekly_reset_at") ||
      null,
    rolling5hResetAt:
      stringAt(value, "rolling5hResetAt") ||
...
function matchRemaining(text: string, pattern: RegExp): Record<string, unknown> | null {
...
function matchPercent(text: string, pattern: RegExp): number | null {
  const match = text.match(pattern);
...
function sourceEnvironment(): string {
