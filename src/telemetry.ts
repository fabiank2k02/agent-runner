...
export class TelemetryIngestError extends Error {
  constructor(
    ...
    readonly body: { error?: string; conflict?: boolean; latestSequence?: number } | undefined
  ) {
    super(message);
...
export function redactSecrets(input: string): string {
  ...
  output = output.replace(
    /\b(OPENAI_API_KEY|CODEX_API_KEY|CODEX_ACCESS_TOKEN|DIGITALOCEAN_TOKEN|DIGITALOCEAN_ACCESS_TOKEN|CLOUDFLARE_API_TOKEN|CLOUDFLARE_TOKEN|GH_TOKEN|GITHUB_TOKEN)\b\s*[:=]\s*["']?[^"'\s,;]+/giu,
    (_match, name: string) => `${name}=${redactedSecretMarker}`
  );
...
export function boundRawPayload(payload: Record<string, unknown>): {
  payload: Record<string, unknown>;
  truncated: boolean;
  truncationReason?: string;
} {
  ...
  if (!bounded.truncated) {
    return { payload: normalized, truncated: Boolean((normalized as { truncated?: boolean }).truncated) };
  }
  return {
    payload: {
      truncated: true,
...
export function localTelemetryStateDir(projectSlug: string): string {
  return path.join(os.homedir(), ".agent-runner", "telemetry", projectSlug);
}

export function localTelemetryStatePath(projectSlug: string): string {
  return path.join(localTelemetryStateDir(projectSlug), "state.json");
}

export function localTelemetryLogPath(projectSlug: string): string {
  return path.join(localTelemetryStateDir(projectSlug), "service.log");
}

export async function readLocalTelemetryState(projectSlug: string, statePath = localTelemetryStatePath(projectSlug)): Promise<LocalTelemetryState> {
  try {
    const raw = await fs.promises.readFile(statePath, "utf8");
    const parsed = JSON.parse(raw) as Partial<LocalTelemetryState>;
    return {
      version: 1,
      projectSlug,
      lastStreamSequence: Number.isInteger(parsed.lastStreamSequence) ? parsed.lastStreamSequence! : 0,
      knownStreams: Array.isArray(parsed.knownStreams) ? parsed.knownStreams.filter((item): item is string => typeof item === "string") : [],
      streams: parsed.streams && typeof parsed.streams === "object" ? parsed.streams : {},
      lastUploadedChunkHashes:
        parsed.lastUploadedChunkHashes && typeof parsed.lastUploadedChunkHashes === "object" ? parsed.lastUploadedChunkHashes : {},
...
export async function writeLocalTelemetryState(state: LocalTelemetryState, statePath = localTelemetryStatePath(state.projectSlug)): Promise<void> {
...
export class WorkspaceTelemetryAdapter implements TelemetrySourceAdapter {
  readonly kind = "local-workspace" as const;

  constructor(private readonly projectRoot: string, private readonly projectSlug: string, private readonly denyGlobs: string[] = []) {}
...
export class CodexCliSessionAdapter implements TelemetrySourceAdapter {
  readonly kind = "codex-cli-thread" as const;

  constructor(private readonly options: { codexHome?: string; projectRoot: string; projectSlug: string; denyGlobs?: string[] }) {}
  ...
  async readDelta(stream: DiscoveredStream, cursor: Cursor | undefined): Promise<TelemetryDelta> {
...
export class SimulatedIdeThreadAdapter implements TelemetrySourceAdapter {
  readonly kind = "codex-ide-thread" as const;

  constructor(private readonly projectSlug: string, private readonly streams: Array<{ id: string; title: string; activity: string }> = []) {}
...
export async function flushLocalTelemetry(
  ...
  const streams: LocalTelemetryFlushResult["streams"] = [];
  let uploaded = 0;
  let skipped = 0;
...
export async function uploadRawTelemetryEnvelope(context: CommandContext, envelope: RawTelemetryEnvelope): Promise<Record<string, unknown>> {
  const endpoint = context.config.dashboard.endpoint;
  if (!endpoint || !hasDashboardAuth(context.config.dashboard)) {
    throw new Error(
      `Telemetry upload requires AGENT_RUNNER_DASHBOARD_ENDPOINT and either ${context.config.dashboard.tokenEnv} or AGENT_RUNNER_CF_ACCESS_CLIENT_ID/AGENT_RUNNER_CF_ACCESS_CLIENT_SECRET.`
    );
  }
  ...
  const { body } = await readDashboardJson<{ error?: string; conflict?: boolean; latestSequence?: number }>(response, "telemetry ingest");
  if (!response.ok) {
...
function ingestConflictLatestSequence(error: unknown): number | null {
  const candidate = error as { status?: unknown; body?: { conflict?: unknown; latestSequence?: unknown } } | null;
...
export async function localTelemetryStatus(context: CommandContext, options: { statePath?: string } = {}): Promise<LocalTelemetryStatus> {
  const statePath = options.statePath || localTelemetryStatePath(context.config.projectSlug);
...
export async function startLocalTelemetryService(context: CommandContext): Promise<TelemetryServiceStartResult> {
  requireDashboardTelemetryConfig(context);
...
export async function stopLocalTelemetryService(context: CommandContext): Promise<TelemetryServiceStopResult> {
  const statePath = localTelemetryStatePath(context.config.projectSlug);
  const state = await readLocalTelemetryState(context.config.projectSlug, statePath);
  if (!state.pid) {
...
export async function runLocalTelemetryService(context: CommandContext): Promise<void> {
...
function requireDashboardTelemetryConfig(context: CommandContext): void {
  if (!context.config.dashboard.endpoint || !hasDashboardAuth(context.config.dashboard)) {
    throw new Error(
      `Telemetry requires AGENT_RUNNER_DASHBOARD_ENDPOINT and either ${context.config.dashboard.tokenEnv} or AGENT_RUNNER_CF_ACCESS_CLIENT_ID/AGENT_RUNNER_CF_ACCESS_CLIENT_SECRET.`
    );
  }
}

function hasDashboardAuth(dashboard: CommandContext["config"]["dashboard"]): boolean {
  return Boolean(dashboard.token || (dashboard.accessClientId && dashboard.accessClientSecret));
}

function defaultSourceId(sourceKind: TelemetrySourceKind, projectSlug: string): string {
  const host = process.env.CODESPACE_NAME || os.hostname();
  return `${sourceKind}:${projectSlug}:${host}`;
}

function localStreamKey(sourceKind: TelemetrySourceKind, streamId: string): string {
  return `${sourceKind}:${streamId}`;
}

function repoLocalTelemetryStatePath(projectSlug: string): string {
...
function normalizePayloadLimits(value: unknown): Record<string, unknown> {
...
async function listJsonlFiles(root: string, limit: number, denyGlobs: string[] = []): Promise<string[]> {
...
export function isDeniedLocalTelemetryPath(input: string, denyGlobs: string[] = []): boolean {
...
function summarizeCodexJsonl(lines: string[]): {
  title?: string;
  status: string;
  latestActivity: string;
  promptSnippets: string[];
  agentMessageSnippets: string[];
  ...
  const agentMessageSnippets: string[] = [];
...
function collectFileReferences(value: unknown, files: Set<string>): void {
...
function stableStreamId(input: string): string {
  const normalized = input.replace(/[^a-zA-Z0-9._-]+/gu, "-").replace(/^-|-$/gu, "").slice(0, 120);
  return normalized || sha256Hex(input).slice(0, 16);
}

function parseDirtyFiles(status: string, denyGlobs: string[] = []): Array<{ path: string; status: string }> {
  return status
    .split(/\r?\n/u)
