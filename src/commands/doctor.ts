...
export async function doctor(context: CommandContext): Promise<DoctorResult> {
  ...
  checks.push({
    ...
    detail:
      config.dashboard.enabled && config.dashboard.endpoint && hasDashboardAuth(config.dashboard)
...
async function cloudflareChecks(context: CommandContext): Promise<DoctorCheck[]> {
  ...
  const accountId = Array.isArray(accounts.result) ? (accounts.result[0] as { id?: string } | undefined)?.id : undefined;
  checks.push({
  ...
  if (bindings.r2Bucket) {
    const r2 = await cloudflareFetch(
      token,
      `/accounts/${accountId}/r2/buckets/${encodeURIComponent(bindings.r2Bucket)}/objects`
    );
    checks.push({
      name: "Cloudflare R2 list/delete permission",
      ok: r2.ok,
...
async function verifyD1Write(token: string, accountId: string, databaseId: string): Promise<{ ok: boolean; detail: string }> {
  ...
  const insert = await cloudflareFetch(token, `/accounts/${accountId}/d1/database/${databaseId}/query`, {
    ...
    body: {
      sql:
        "INSERT OR REPLACE INTO processing_leases (id, owner_id, acquired_at, expires_at, heartbeat_at, metadata_json) VALUES (?, ?, ?, ?, ?, ?)",
...
async function cloudflareFetch(
  ...
  try {
    ...
    const body = await response.json() as { success?: boolean; result?: unknown; errors?: Array<{ code?: number; message?: string }> };
...
function readDashboardBindings(projectRoot: string): { databaseId?: string; r2Bucket?: string } {
  try {
