...
async function validatePullTarget(context: CommandContext): Promise<void> {
  ...
  await remote.run(
    `[ -d ${quoteRemotePath(layout.remoteProjectDir)} ] || { echo "Remote project directory not found: ${layout.remoteProjectDir}" >&2; exit 44; }`
  );
}
