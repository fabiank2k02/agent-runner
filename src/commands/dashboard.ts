...
export function assertDashboardLaunchConfig(context: CommandContext): void {
...
export async function startDashboardObserver(
  ...
  try {
    ...
    throw new DashboardLaunchError(
      `Dashboard observer did not publish verified telemetry for ${layout.projectSlug}:${task.taskId}: ${
...
export function dashboardJobsUrl(endpoint: string): string {
