...
export function extractLiveEvents(logText, options = {}) {
...
export function aggregateFileActivity(events, limit = 120) {
    const files = new Map();
...
export function extractTokenUsage(logText) {
    const usage = {
        inputTokens: 0,
        cachedInputTokens: 0,
        outputTokens: 0,
...
export function calculateSubscriptionSpend(input) {
    ...
    const digitalOceanCostUsd = elapsedMinutes === null || digitalOceanHourlyUsd === null ? null : digitalOceanHourlyUsd * (elapsedMinutes / 60);
    ...
    const codexWeeklyBudgetUsd = codexSubscriptionMonthlyUsd === null
        ? null
    ...
    const codexWeeklyBudgetFormula = codexWeeklyBudgetUsd === null
        ? null
    ...
    let codexTaskAllocationUsd = null;
    let codexAllocationMethod = "unknown";
    ...
    const codexTaskAllocationPercent = codexTaskAllocationUsd === null || codexWeeklyBudgetUsd === null || codexWeeklyBudgetUsd === 0
        ? null
        : (codexTaskAllocationUsd / codexWeeklyBudgetUsd) * 100;
    const codexRemainingWeeklyBudgetUsd = codexTaskAllocationUsd === null || codexWeeklyBudgetUsd === null
        ? codexWeeklyBudgetUsd
        : Math.max(0, codexWeeklyBudgetUsd - codexTaskAllocationUsd);
    const knownParts = [digitalOceanCostUsd, codexTaskAllocationUsd].filter((value) => typeof value === "number");
...
export function deriveGoalsFromPrompt(prompt, limit = 12) {
...
export function deriveSubgoalsFromEvents(events, limit = 12) {
    const subgoals = [];
...
export function currentActivityFromEvents(events, fallback = "Waiting for structured activity.") {
    const event = events
        .slice()
        .reverse()
...
export function telemetryRuntimeSource() {
    return [
        "const MAX_LABEL_CHARS = 120;",
        "const MAX_DETAIL_CHARS = 500;",
        "const DEFAULT_EVENT_LIMIT = 200;",
...
function eventsFromPlainLine(line, lineNumber) {
...
function eventsFromJson(value, rawLine, lineNumber) {
...
function classifyItemEvent(type, item, timestamp, source) {
    const events = [];
...
function eventsFromCommand(command, timestamp, source) {
    ...
    return paths.map((filePath) => createEvent({
        timestamp,
        type,
        label,
        detail: clipText(command, MAX_DETAIL_CHARS),
...
function eventsFromPatchText(patchText, timestamp, source) {
    const files = [];
...
function extractPatchText(item) {
...
function goalFromLabel(label, source) {
...
function formatEventAction(type) {
    if (type === "file_read") {
