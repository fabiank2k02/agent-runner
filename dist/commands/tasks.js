...
import { DashboardLaunchError, assertDashboardLaunchConfig, startDashboardObserver } from "./dashboard.js";
export function createTaskId(date = new Date()) {
    const timestamp = date.toISOString().replace(/[-:]/gu, "").replace(/\..+$/u, "Z");
    return `${timestamp}-${randomBytes(3).toString("hex")}`;
}
export function sessionName(projectSlug, taskId) {
    return `agent-runner-${projectSlug}-${taskId}`.replace(/[^a-zA-Z0-9._-]/gu, "-").slice(0, 180);
}
export async function runTask(context, prompt, options = {}) {
...
export async function taskStatus(context, taskId) {
    ...
    const status = await context.remote.run(`[ -f ${quoteRemotePath(task.statusFile)} ] && cat ${quoteRemotePath(task.statusFile)} || true`);
    return status.stdout || JSON.stringify({ taskId: task.taskId, sessionName: task.sessionName, status: tmux.stdout || "unknown" });
}
export async function taskLogs(context, taskId, lines) {
    const task = await resolveTask(context, taskId);
    const command = lines
...
function buildAppServerClientScript(context, taskId) {
    const config = {
